...
type WishlistCardProps = {
  ...
  onCardClick: (item: WishlistItemDto) => void;
};

export const WishlistCard = ({ item, onRemove, onCardClick }: WishlistCardProps) => {
  const { product } = item;
  ...
  return (
    <div className="group overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:scale-[1.02]">
      ...
      <div className="space-y-1.5 p-3">
        <h3
          className="line-clamp-1 cursor-pointer text-sm leading-tight font-semibold"
          onClick={() => onCardClick(item)}
        >
          {product.title}
        </h3>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {display.isOnSale && (
            <span className="text-xs text-red-500 line-through">{formatPrice(display.price)}</span>
          )}
        </div>
      </div>
    </div>
